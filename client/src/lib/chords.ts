...
const rootsLevel7 = allRoots; // All 12 roots for level 7
...
rootsLevel1.forEach((rootNum) => {
  ...
  if (minorPattern) {
    chordsByDifficulty["level1"].push(
      createChordDef(rootNum, minorPattern, idCounter++, "level1"),
    );
  }
});
...
if (patterns.dim) {
  chordsByDifficulty["level7"].push(
    createChordDef(3, patterns.dim, idCounter++, "level7"),
  ); // Ddim
}

if (patterns.sus2) {
  chordsByDifficulty["level7"].push(
    createChordDef(1, patterns.sus2, idCounter++, "level7"),
  ); // Csus2
}

if (patterns.sus4) {
  chordsByDifficulty["level7"].push(
    createChordDef(3, patterns.sus4, idCounter++, "level7"),
  ); // Dsus4
}

if (patterns.majorInv1) {
  chordsByDifficulty["level7"].push(
    createChordDef(1, patterns.majorInv1, idCounter++, "level7"),
  ); // C/E
}

if (patterns.minorInv1) {
  chordsByDifficulty["level7"].push(
    createChordDef(10, patterns.minorInv1, idCounter++, "level7"),
  ); // Am/C
}

if (patterns.majorInv2) {
  chordsByDifficulty["level7"].push(
    createChordDef(8, patterns.majorInv2, idCounter++, "level7"),
  ); // G/D
}

if (patterns.minorInv2) {
  chordsByDifficulty["level7"].push(
    createChordDef(3, patterns.minorInv2, idCounter++, "level7"),
  ); // Dm/A
}
...
export function checkChordMatch(
  ...
  return sortedSelected.every(
    (noteNum, index) => noteNum === chordNoteNumbers[index],
  );
}
